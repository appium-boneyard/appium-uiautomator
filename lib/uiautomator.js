...
class UiAutomator extends events.EventEmitter {
  ...
  async start (uiAutomatorBinaryPath, className, extraParams, startDetector) {
    let processIsAlive;
    if (!startDetector) {
      startDetector = null;
    }
    try {
      ...
      log.info("Starting UiAutomator");
      log.info("Parsing uiautomator jar");
      ...
      // for bootstrapJar
      // jarName = bootstrap.jar
      // className = io.appium.android.bootstrap.Bootstrap
      // extraParams = "-e pkg appPackage -e disableAndroidWatchers true/false"
      // startDetector = (stdout) => {
      ...
      log.info(`Executing command ${adbPath} ${args}`);
